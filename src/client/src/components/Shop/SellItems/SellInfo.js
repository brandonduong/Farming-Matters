...
export const SellInfo = ({name, inventoryState, allTurnPrices, turn, itemSelected, setItemSelected, cropInfo, money, setMoney}) => {
    let insuredCrops = {}
    ...
    const sell = () => {
        ...
        Object.entries(selectedQuantities).forEach(([floorPrice, qty]) => {
            const parsedFloorPrice = floorPrice == 'null' ? null : parseInt(floorPrice)

            let crops = cropInfo[name].filter(({floorPrice: p}) => p == parsedFloorPrice)
            let insuranceInfo = crops.sort((a, b) => a.cropExpiry - b.cropExpiry)
            
            const expiry = insuranceInfo.length > 0 ? insuranceInfo[0].cropExpiry : undefined
            ...
            { name == "" ? (
                ...
                <>
                    ...
                    <div className="details">
                        <div className={"price " +(turn == 1 ? "turn-1" : getIsPriceIncrease() ? "price-increase" : "price-decrease")}>
                            Current Price: ${parseFloat(price).toFixed(2)}  {turn == 1 ? ("-") : getIsPriceIncrease() ? (<span>&#8593;</span>): (<span>&#8595;</span>)}
                        </div>
                    </div>
