...
const SeasonTransition = (props) => {
  ...
  function findStatement() {
    for (let i = 0; i < seasonTransition.length; i++) {
      console.log(
        'HERE: ',
        props.typeOfCatastrophicEvent,
        Object.keys(seasonTransition[i])[0],
        'CURRENT SEASON: ',
        props.season,
      );
      if (
        props.typeOfCatastrophicEvent == Object.keys(seasonTransition[i])[0]
      ) {
      ...
      {true ? (
        <>
          <div className="season-transition">
            <img
              className={'background-img-' + props.season.toLowerCase()}
              src={
                props.typeOfCatastrophicEvent == 'SnowStorm'
                  ? winterImg
                  : props.typeOfCatastrophicEvent == 'HeavyRain'
                  ? heavyRainImg
                  : props.typeOfCatastrophicEvent == 'Drought'
                  ? underwaterImg
                  : props.typeOfCatastrophicEvent == 'Tornadoes'
                  ? torandoImg
                  : ''
              }
            ></img>
            <div className="season-transition-component">
              <div className="transition-box">
                ...
                <div className="warning-msg-box">
                  <div className="warning-msg">"{findStatement()}"</div>
                </div>
              </div>
              <button
                type="button"
                id="season-transition-close"
                className="close-button"
                onClick={() => exitTransition()}
              >
                x
              </button>
            </div>
