export function addItemToCropInfo(cropInfo, item) {
  ...
  for (let i = 0; i < cropContracts?.length; i++) {
    ...
    if (
      currContract.floorPrice == itemFloorPrice &&
      currContract.cropExpiry == itemCropExpiry
    ) {
      match = currContract;
      matchIndex = i;
      break;
...
export function removeItemFromCropInfo(cropInfo, item) {
  ...
  if (match != '' && matchIndex != null) {
    if (currCount >= 2) {
      ...
      cropInfo[itemName] = cropInfo[itemName].filter(
        (item) =>
          item.floorPrice != itemFloorPrice ||
          item.cropExpiry != itemCropExpiry,
      );
    }
  }
}
...
export function removePlant(plantedSeeds, seed) {
  ...
  let coordx = seed.coords.x;
  let coordz = seed.coords.z;
  let index = plantedSeeds.findIndex(
    (seed) =>
      seed.type == type &&
      seed.name == seedName &&
      seed.floorPrice == floorPrice &&
      seed.coords.x == coordx &&
      seed.coords.z == coordz,
  );
